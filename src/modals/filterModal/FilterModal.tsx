...
export const FilterModal: React.FC<FilterModalProps> = ({
    ...
    useEffect(() => {
        const unsubscribeCategories = getAllCategories((data) => setCategories(data));
    ...
    const handleApplyFilters = () => {
        Object.keys(tempFilters).forEach(key => {
            if (key === 'dateRange') {
                dispatch({ type: 'SET_DATE_RANGE', field: 'start', value: tempFilters.dateRange.start });
                dispatch({ type: 'SET_DATE_RANGE', field: 'end', value: tempFilters.dateRange.end });
            } else {
                dispatch({ type: 'SET_FILTER', field: key as keyof typFilters, value: tempFilters[key as keyof typFilters] });
            }
        });
        onClose();
    };
